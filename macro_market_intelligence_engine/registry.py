...
def default_registry() -> MacroMarketRegistry:
    ...
    registry.register_asset(
        AssetRegistration(
            ...
            mechanism_template=(
                "{symbol}作为{role}，核心传导始终围绕流动性和风险偏好："
                "一旦流动性收紧，风险偏好下降会先压缩小盘高β资产的承接资金，"
                "随后更容易触发被动抛售与做市深度下降，资金撤离会放大价格冲击和日内波动。"
                "当前环境表现为{liquidity_phrase}和{risk_phrase}，因此它的仓位含义主要取决于资金承接能力是否改善，"
                "而不是单一基本面叙事。"
            ),
        )
    )
